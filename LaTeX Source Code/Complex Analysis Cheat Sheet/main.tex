...
\node [mybox] (box){%
    ...
    \textbf{Some basic results:}
       \begin{itemize}
           \item For $z_0 \in \C, r>0$ we denote the ball (i.e. disk) of radius $r$ around $z_0$ to be $D(z_0,r)=\{ z \in \C \mid |z-z_0| < r\}$
           \item A point $z \in \C$ is a \textbf{limit point} of $E \subseteq \C$ if $\forall \varepsilon>0$, $D(z,\varepsilon)\cap E$ contains a point other than $z$.
           \item A subset $E \subseteq \C$ is said to be \textbf{open} if $\forall z \in E, \exists\ r>0$, s.t. $D(z,r)\subset E.$
...
\node [mybox] (box){%
    ...
    \end{itemize}
    \textbf{Reparametrization:} We say a curve $\gamma_2:[a_2,b_2]\rightarrow \C$ is a continuous reparametrization of $\gamma_1:[a_1,b_1] \rightarrow \C$, if there exists a homeomoprhism $\varphi: [a_1,b_1] \rightarrow [a_2,b_2]$ s.t.$\varphi(a_1)=a_2, \varphi(b_1)=b_2$ and $\gamma_2(\varphi(t))=\gamma_1(t) \forall t \in [a_1,b_1]$.
    \begin{itemize}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			Let $ \Omega =\{z\in\Omega: a < \Re(z) < b\} $. Let $ f:\overline{\Omega} \rightarrow \C $, s.t. $ f $ is continous on $ \overline{\Omega} $ and holomorphic on $ \Omega $. Suppose for some $ z=x+iy $, we have $|f(z)|<B $ and let\\
