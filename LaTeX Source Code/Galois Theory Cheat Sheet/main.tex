...
\begin{tikzpicture}
	\node [mybox] (box){%
		...
		\begin{itemize}
			...
			\item All $ g \in \text{Gal} {L/K}$ acts as $\zeta \rightarrow \zeta^{a^g}, (a^g,n) =1 $.
			\item $ \text{Gal}(L/K)$ injects into $ \Z/n\Z^\times $ and this is an isomorphism when $ \Phi_n  $ is irreducible over $ K $.
		\end{itemize}
		\end{minipage}
	};			
	%------------ Cyclotomic polynomial Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Cyclotomic polynomials};
\end{tikzpicture}
